...
def undistort_advanced(balance=0.0, dim2=None, dim3=None):#img_path, balance=1.0, dim2=None, dim3=None):
    #img = cv2.imread(img_path)
    print("starting stream..")
    vcap = cv2.VideoCapture("C:/Users/DEU217504/Videos/Viofo/output_test_480p.mp4")#"rtsp://192.168.1.254")
    vcap.set(cv2.cv2.CAP_PROP_POS_FRAMES, 3500)
    #vcap.set(cv2.CAP_PROP_BUFFERSIZE, 1)
    print("stream started")
    dim1 = DIM#(848,480)#img.shape[:2][::-1]  #dim1 is the dimension of input image to un-distort
    ...
    while(1):
        ...
        #undistorted_img = cv2.resize(undistorted_img, , interpolation = cv2.INTER_AREA)

        # fourcc = cv2.VideoWriter_fourcc(*'mp4v')
        # out = cv2.VideoWriter('outpy.mp4',fourcc, 25.0, DIM)
        # out.write(undistorted_img)
        
        print("framecount: "+str(count))
        count = count+1
        cv2.imwrite("undistorted_"+str(count)+".jpg", undistorted_img)
